...
metadata = {
    'protocolName': 'Protein titration - 24 well, add fluor after',
    'author': 'Shawn Laursen',
    'description': '''You put mixes in 96 well plate: 
                        - 30ul of protein col 0 
                        - 280ul of dilutant col 1 
                        - 280ul of fluor col 2
                      Robot:
                        - Titrates protein 23 times in 384well 
...
def protein_titration(buff, protocol):
    ...
    p20m.blow_out()
    p20m.aspirate(10, plate384.rows()[which_rows][start_384well+22])
    p20m.drop_tip()
    p300m.pick_up_tip()
    p300m.aspirate(260, plate96.rows()[0][fluor_col])
    for j in range(start_384well, start_384well+24):
