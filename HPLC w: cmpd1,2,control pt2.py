...
def HPLC(start_96well,  RXN_time, protocol):
    ...
    KOH = trough.wells_by_name()['A6']
     ...
     # Slice Names
    spun_noHPF1 = slice(0,3)
    spun_HPF1 = slice(3,6)
    rows_noHPF1 = slice(0,3)
    rows_HPF1 = slice(3,6)
    ...
    p300m.distribute(17, KOH, newplate96.rows()[0][slice1], disposal_volume = 0, new_tip = 'never')
    shaker.set_and_wait_for_shake_speed(1000)
    protocol.delay(minutes=1)
    shaker.deactivate_shaker()

    p300m.distribute(17, KOH, newplate96.rows()[0][slice2], disposal_volume = 0, new_tip = 'never')
    shaker.set_and_wait_for_shake_speed(1000)
    protocol.delay(minutes=1)
    shaker.deactivate_shaker()
